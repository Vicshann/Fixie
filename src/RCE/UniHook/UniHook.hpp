...
{
  __builtin___clear_cache((char *)addr, (char *)((size_t)addr + len));  // LIBC::__aarch64_sync_cache_range()
}

static bool sh_util_is_thumb32(size_t target_addr)
{
...
static int sh_util_write_inst(size_t target_addr, void *inst, size_t inst_len)
{
...
{
 this->impl.enter_addr = this->Stub;     // This is called as a proc
 DBGDBG("Before: %08X", *(uint32*)this->TProcAddr);
#if defined(CPU_ARM) 
 int res = this->impl.sh_inst_hook(this->TProcAddr, this->HProcAddr);
#else
 int res = this->impl.InstallHook(this->TProcAddr, this->HProcAddr, this->Flags);
#endif
...
bool IsSet(void)
{
 if(!this->HProcAddr || !this->TProcAddr)return false;
 return this->impl.IsActiveAt(this->TProcAddr);
}
//------------------------------------------------------------------------------------------------------------
// TODO: Near allocation of tramopline stubs (static)
int Set(vptr TgtProcAddr, vptr HookProcAddr=HAddr, uint Flags=Flg)
{
 if(this->HProcAddr && this->TProcAddr)return -1;
 this->Flags     = Flags;
 this->TProcAddr = TgtProcAddr;
 this->HProcAddr = HookProcAddr;
 int res = this->InstallInternal();
 if(res)this->TProcAddr = nullptr;
 return res;
}
//------------------------------------------------------------------------------------------------------------
...
{
#if defined(CPU_ARM) 
 return this->impl.sh_inst_unhook(this->TProcAddr);
#else
 return this->impl.Remove(this->TProcAddr);
#endif
}
//------------------------------------------------------------------------------------------------------------
};
...
{
 //return MemProtect(this, sizeof(T), PX::PROT_READ | PX::PROT_WRITE | PX::PROT_EXEC) == 0;   // May break something if the original protection is not preserved
 return true;
}
//------------------------------------------------------------------------------------------------------------
