...
#endif
 if constexpr (sizeof(T) > sizeof(uint32))return (unsigned int)__builtin_clzll((uint64)Num);  // X64 CPUs only?   // Returns the number of leading 0-bits in x, starting at the most significant bit position. If x is 0, the result is undefined
...
if(!Num)return BitSize<T>::V;  // Every bit is zero
#endif
 if constexpr (sizeof(T) > sizeof(uint32))return (unsigned int)__builtin_ctzll((uint64)Num);  // X64 CPUs only?
...
// MSVC-X86: __popcnt16() / __popcnt() / __popcnt64()
//
