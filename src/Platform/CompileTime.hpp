...
SCVR uint64 DateEKey = NCRYPT::CRC32(__DATE__);  // TODO: CRC64!!!
...
// T: Actual array element type
//
template<typename V, typename T, usize N> struct CPData    // alignas(8) // !!! Cannot be used as a function argument: argument deduction not allowed in a function prototype
{
 SCVR usize ChrInUnit = sizeof(usize) / sizeof(V);      // !!! Cannot pack u64 on a x32 system!
...
template<typename Tag, auto From = int{}, auto Step = int{1}, auto U = []{}> [[nodiscard]] consteval auto ctCtr(void)  // NOTE: Not bound to any instance    // TODO: Limit to Max, split recursion by range groups
{
...
{
 template<auto From = int{}, auto Step = int{1}, auto U = []{}> [[nodiscard]] static consteval auto Next(void)   // May step from 1 by 2 if needed
  {
