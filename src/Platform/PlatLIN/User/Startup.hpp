...
static sint InitStartupInfo(vptr StkFrame=nullptr, vptr ArgA=nullptr, vptr ArgB=nullptr, vptr ArgC=nullptr)  // Probably should be private but...
{
 ...
 if(ArgA && !((usize)ArgA ^ (usize)ArgB) && !((usize)ArgA ^ (usize)ArgC)){StkFrame = ArgA; ArgA=ArgB=ArgC=nullptr; fwsinf.Flags |= sfLoadedByLdr;}   // Magic: Real stack frame ptr is passed in all 3 args
 ...
 if(ArgB && ArgC && (((size_t*)ArgC) == (((size_t*)ArgB)+((size_t)ArgA)+1)) && (((size_t*)ArgB)[-1] == (size_t)ArgA))   // System entry point does not receive ArgC,ArgV,EnvP but LD passes those to _init
  ...
  // vptr optr = StkFrame;
   ...
   if(Frame && !Frame[0] && !Frame[1] && !Frame[2] && !Frame[3])
    {
     Frame = (size_t*)UNIX::FindStartupInfoByAuxV(Frame);  // ArgC,ArgV,EnvP,AuxV are null - probably incorrectly detected because of 4 null entries on stack
     if(Frame)
      {
       fwsinf.Flags |= sfLoadedByLdr;    // Too far and NULLs on stack are put by a loader most likely
