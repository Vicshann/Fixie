...
FUNC_WRAPPERNI(NTHD::spawn,       spawn       )         // TODO: !!! Create as a child, not sibling so that waitpid could work!!!
{
 ...
 volatile size_t* Configs  = (volatile size_t*)GetParFromPk<3>(args...);
     ...
     switch((uint16)type)
      {
       ...
       case uint16(NTHD::CfgPsCurDir):
        AltCurDir = (volatile achar*)Configs[1];
        break;
...
//  volatile size_t* ConfigsDup = (size_t*)GetParFromPk<3>(args...);  
  if(Configs)
   {
    for(size_t type=0;(type=*Configs);Configs+=(type >> 16)+1)
     {
      if(type == NTHD::CfgPsFDShare)SAPI::dup3(Configs[1], Configs[2], 0);
        else if(type == NTHD::CfgPsFDShareCoE)SAPI::dup3(Configs[1], Configs[2], PX::O_CLOEXEC);
