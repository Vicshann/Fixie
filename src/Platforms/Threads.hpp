...
// Must be two NULLs and a hack for Clang is required
#define TERMARGLST nullptr,STR_NULL
#define TERMFLGLST 0,size_t(STR_NULL)
...
enum EPrThCfg           // May be applicable to process creation too
{
 ...
 CfgPsFDShare    = (2 << 16) | 8,     // A pair follows {oldfd,newfd}  // On Windows maps only STDIN, STDOUT, STDERR to PEB anything else just duplicated
 CfgPsFDShareCoE = (2 << 16) | 9,     // Same as CfgPsFDShare but with close-on-exec if supported
 CfgSetTgtCurDir = (0 << 16) | 10,    // Set the current directory as the target application path (Takes priority over CfgPsCurDir)
 CfgNoPathInArg0 = (0 << 16) | 11,    // Do not add the executable path in Arg0 (i.e. you already passed some path there)
}; 
...
static PX::pid_t  PXCALL thread(PThreadProc Proc, PX::PVOID Data, PX::SIZE_T DatSize, PX::PSIZE_T Config);     // Improvised  // Config is pairs of {Type,Value} or single {Flag}  // Actually allocates: PageAlign(Size+StkSize+TlsSize+ThreadRec)
...
//
static PX::pid_t  PXCALL spawn(PX::PCCHAR path, PX::PPCHAR argv, PX::PPCHAR envp, PX::PSIZE_T Config);    // Improvised  // int siofd[3]{STDIN,STDOUT,STDERR} // Flags are additional flags for Clone, unused for now
